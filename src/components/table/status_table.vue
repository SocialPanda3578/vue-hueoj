      ...
      <table>
        ...
        <tbody v-if="result.length > 0">
          <tr v-for="item in result" :key="item.id">
            ...
            <td class="status">
              <a :href="`/status/${item.id}`" class="status" :class="{ AC_status: item.result.status === 'Accept' }">
                {{ item.result.status }}
              </a>
            </td>
            <td>{{ item.lang }}</td>
            ...
            <td><div>{{ formatUTC8Date(item.time) }} {{ formatUTC8Time(item.time) }}</div></td>
          </tr>
        </tbody>
        <tbody v-else>
          <tr>
...
export default {
  ...
  methods: {
    async fetchData(page = this.currentPage, size = this.pageSize) {
      try {
        const response = await axios.get(`${this.apiURL}/api/act/list/submission/?page=${page}&size=${size}`, {
          headers: {
